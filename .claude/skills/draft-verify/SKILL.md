...
## 1.8 관객 통독 (설치 전 필수 — 2026-08-19)

수치·오디오 게이트는 **조각이 있는지**만 잰다. 조각이 이해되는지는 통독만이 잰다 — Housemaid ending은
모든 게이트를 통과한 채로 "무슨 소리인지 모르겠는" 컷을 설치했다(훅 미회수·알리바이 프레임 부재·장례식
시임 부재). 설치 전에 `read_story.js`(스크래치패드)로 원고 전문을 뽑아 **영화를 안 본 사람** 기준으로
읽고, 다섯 질문에 전부 답할 수 있어야 설치한다:

1. **훅 회수**: 콜드오픈 대사가 나중에 그 대화/모먼트로 돌아와 닫히는가? (기계 게이트
   `cold-open hook is never paid off`가 플랜 수준에서 잡지만, 원고 수준에서 재확인)
2. **모든 대사 조각이 착지하는가**: 각 대사가 "누가·누구에게·왜"를 앞 문맥에서 알 수 있는가?
   특히 암시형 대사("모든 게 완벽해야 했잖아") 앞에 프레임 문장이 있는가?
3. **장면 점프마다 시임**: 장소/시간이 바뀌는 지점에 나레이션·카드가 있는가?
4. **이름-얼굴**: 처음 말하는 인물마다 나레이션이 미리 소개했는가?
5. **결말**: 마지막 문장이 훅/전제를 딛는가, 아니면 방금 본 걸 재진술하는가?

하나라도 답이 안 나오면 **설치하지 말고** compress fills/plan을 수술한다. 암시형 대사가 많은 소스
(스릴러·복선형)는 시임 나레이션을 구조 위치가 아니라 **대사의 컨텍스트 의존도**에 따라 늘린다.
