...
## VAD를 트림 층에 배선하지 않기로 한 근거 (2026-08-16)

에너지 검출기(silencedetect)가 566초 중 **128.7초를 발화로 오판**한다는 사실만 보면 트림을
silero-vad로 갈아끼워야 할 것 같지만, 실제 클립 경계를 재보면 그 오차는 대부분 우리가 자르는 지점
바깥에 있다(`report_clip_dead_air.js`):

| 소스 | 앞 무발화 p50/p90 | 뒤 무발화 p50/p90 | 총 무발화 |
|---|---|---|---|
| allin | 0.05 / 0.15s | 0.38 / 0.97s | 11.4s (VAD가 소리로 듣는 건 2.5s뿐) |
| outsmart | 0.05 / 0.11s | 0.17 / 1.75s | 15.7s |
| ending | 0.05 / 0.13s | 0.22 / 1.18s | 11.2s |
| night | 0.03 / 0.14s | 0.87 / 1.79s | 23.9s |
| longshot | 0.05 / 1.46s | 0.20 / 1.78s | 23.5s |

앞은 이미 촘촘하고(첫 단어까지 0.05초), 무발화는 대부분 **뒤 꼬리 = 의도된 리액션 테일**이다.
배선하면 5개 소스를 다시 렌더·검증·설치해야 하는데 얻는 게 그 정도라 **하지 않는다**. VAD는 계속
**리포트 도구**로 둔다(`detect_speech_ranges.py --compare`). 음악이 깔린 새 소스에서 앞 무발화가
커지면 그때 재검토할 근거가 이 표다.

## "말하는 사람이 화면에 있는가"를 자동 판정하지 않기로 한 근거 (2026-08-16)

입 모션으로 발화자 존재를 근사해 보려 했으나 **만들지 않기로 했다**:

- 이 환경의 opencv 5.0.0(headless)에는 `CascadeClassifier`가 없다(`objdetect` 미포함). 남은 길은
  `FaceDetectorYN` + yunet ONNX 모델 **별도 다운로드**뿐 — 새 의존성이 생긴다.
- 영화 푸티지에서 신호가 지저분하다: 측면·클로즈업·한 클립 안의 컷·복수 인물.
- 무엇보다 **리액션 샷은 하우스 스타일상 정당한 선택**이라, "발화자가 안 보임"이 곧 결함이 아니다.
  즉 자동 실패로 만들 수 없고, 경고로 만들면 소음만 는다.
- 대신 `build_review_contact_sheet.js`가 **수치가 가장 약한 클립 5개만** 프레임으로 뽑아준다.
  이 판단은 사람 눈이 하되, 스물다섯 개가 아니라 다섯 개만 본다.

나중에 다시 시도한다면: yunet 모델을 받아 `FaceDetectorYN`으로 얼굴을 잡고, 얼굴 하단 1/3의
프레임 간 변화량을 재되, **발화 클립과 리액션 클립이 실제로 갈리는지 먼저 확인**할 것(그 확인 없이
게이트에 넣지 말 것).
