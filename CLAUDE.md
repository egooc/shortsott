...
- **What actually generates today: Highlight, plus the TTS Full lanes (`kr_full` approved 2026-08-11, `ja_full` 2026-08-12).** Items marked `production_lane: 'kr_full'` / `'ja_full'` (daily harvest, `harvest_config.json` locale_plan `lane` field; current plan 2026-08-12: JP highlight 8 + ja_full 4 + kr_full 4 + KR highlight 4 = 20/day) produce the TTS-narrated Full draft in the lane's language — each channel mixes highlights (volume, non-verbal reach) with Full (audio language signal). Every other item stays highlight-only: `effectiveDraftVariantModeForItem` returns `'full_only'` only for those lanes, `wantsFullDraft` derives from it, `wantsMidformDraft` stays hardcoded false — all asserted by `check:shortform-highlight`. Each queue item produces drafts in its own `target_locale`, not both languages.
  ...
  - Full-lane machinery is language-parameterized (`fullScriptLanguage`, `validationOptions.fullScriptField`): script fields `full_caption_script_ko`/`_ja`, speech rates MEASURED 6.03775 (ko) / 4.5 (ja) chars/sec, sentence standard = complete spoken sentences (KO 15-25 chars /20 divisor, JA 12-22 /16), lenient machinery (soft accept, budget clip, cycle collapse, scene-id remap, minItems 3) keyed off the lane field. Post-build gate `verifyKoreanFullDraftTimelineAlignment` fails misaligned or narration-gutted drafts. Talking-head windows (face>=0.34 OR speech>=0.5 AND motion<15, `scripts/window_face_probe.py`) are dropped before full-draft concat.
