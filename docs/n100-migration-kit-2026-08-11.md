# N100 미니PC 이전 킷 (2026-08-11, 2026-08-12 갱신)

전용 상시가동 미니PC로 전체 자동화(03:00 수집·배치 / 09:45 내보내기·업로드)를
이전하기 위한 체크리스트.

**2026-08-12 체제**: 4레인 20소재/일 — JP 하이라이트 8 + JA Full(TTS) 4 +
KR Full(TTS) 4 + KR 하이라이트 4. Full 레인은 ElevenLabs TTS를 쓰므로
`.env`의 `ELEVENLABS_API_KEY` 필수. 장면 필터(`scripts/window_face_probe.py`)가
`tools/models/face_detection_yunet_2023mar.onnx` + silero-vad + opencv를 쓴다.
pyautogui FAILSAFE는 꺼져 있고(무인용) 내보내기 스크립트는 단계별 픽셀
검증+재시도로 강화됨 — 단, **새 해상도 좌표 프로파일 보정은 여전히 필수**.
09:45 내보내기는 "마지막 완료 잡"을 잡으므로 테스트 잡을 아침에 돌리면
오배송 위험 — 이전 후에는 테스트 배치를 오후에만 돌릴 것.
